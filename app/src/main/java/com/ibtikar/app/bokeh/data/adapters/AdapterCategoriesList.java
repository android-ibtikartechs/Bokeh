...
public class AdapterCategoriesList extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public AdapterCategoriesList(ArrayList<CategoryItemModel> arrayList, Context context) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        final CategoryItemModel categoriesListItem = arrayList.get(position);
        final ItemViewHolder itemViewHolder = (ItemViewHolder) holder;

        if (!(categoriesListItem.getImUrl().equals("") || categoriesListItem.getImUrl() == null ))
        {
            Glide.with(context)
                    .load(categoriesListItem.getImUrl()).diskCacheStrategy(DiskCacheStrategy.ALL)
    ...
    public void add(CategoryItemModel r) {
        arrayList.add(r);
        notifyItemInserted(arrayList.size()-1 );
    }

    public void addAll(ArrayList<CategoryItemModel> opResults) {
        for (CategoryItemModel result : opResults) {
            add(result);
        }
    }
